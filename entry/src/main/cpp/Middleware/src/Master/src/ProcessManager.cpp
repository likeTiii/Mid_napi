  ...
  void ProcessManager::run() {
    ...
    try {
        ...
        if (!std::filesystem::exists(execpath)) {
            throw std::runtime_error("execpath not exists");
        }
    } catch (...) {
        execpath = "/data/local/tmp"; // fallback for OHOS
    }
    ...
    //m_signalSet.async_wait(std::bind(&ProcessManager::onSignal, this, std::placeholders::_1));
    m_signalSet.async_wait(
        [this](const boost::system::error_code &ec, int signal_number) { this->onSignal(ec, signal_number); });
    std::thread{
      [&]{
        ioc.run();
      }
