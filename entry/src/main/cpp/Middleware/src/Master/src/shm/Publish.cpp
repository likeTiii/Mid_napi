...
namespace Hnu::Middleware {
  ...
  bool Publish::run() {
    ...
    int flags = fcntl(m_eventfd, F_GETFD);
    if (flags == -1) {
        spdlog::error("Invalid m_eventfd after pidfd_getfd");
    } else {
        spdlog::debug("Successfully got m_eventfd: {}", m_eventfd);
    }
    ...
    try{
      ...
      m_shm=interprocess::managed_shared_memory(interprocess::create_only,shmName.c_str(),SHM_SIZE,0,perm); //给共享内存设置权限
    ...
    spdlog::debug("Eventfd handle: {}, stream handle: {}", m_eventfd, m_eventfdStream->native_handle());
    doEventfdRead();
    return true;
  }
...
//     m_eventfdStream->async_read_some(
//         asio::buffer(&m_eventfdValue, sizeof(m_eventfdValue)),
//         std::bind(&Publish::onEventfdRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2));   
  }
  void Publish::onEventfdRead(const boost::system::error_code& ec,std::size_t bytes) {
    if(ec){
      ...
      spdlog::error("Eventfd Read Error: {} (errno: {})", ec.message(), ec.value());
      return;
    }
