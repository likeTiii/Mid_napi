...
//     // 在构造函数中初始化 Acceptor，只初始化一次
//     m_acceptor = std::make_unique<Acceptor>(m_ioc);
//   }
  ...
  void MiddlewareManager::run() {
    ...
    //acceptor.run();
    if (!middlewareManager.m_acceptor) {
        middlewareManager.m_acceptor = std::make_unique<Acceptor>(middlewareManager.m_ioc);
        middlewareManager.m_acceptor->run();
    }
