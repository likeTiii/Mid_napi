...
namespace gazebo
{
  class GuiMouseClickGoal : public GUIPlugin
  {
    ...
    GuiMouseClickGoal() : GUIPlugin()
    {
      ...
      this->publisher = node->createPublisher<TaskPlanner::GlobalPathPlan>("global_path");
      this->resize(50, 50);
    ...
    private:
      ...
      bool OnLeftRelease(const common::MouseEvent &event)
      {
        ...
        TaskPlanner::GlobalPathPlan msg;
        
        auto pose = msg.mutable_pose_array()->add_poses();
        pose->mutable_position()->set_x(start_pos.X());
        pose->mutable_position()->set_y(start_pos.Y());
        pose->mutable_position()->set_z(start_pos.Z());
        pose->mutable_orientation()->set_x(quat.X());
        pose->mutable_orientation()->set_y(quat.Y());
        pose->mutable_orientation()->set_z(quat.Z());
        pose->mutable_orientation()->set_w(quat.W());

        msg.set_stop_at_last_point(true);
        msg.set_back_up(false);
    ...
    private:
      ...
      std::shared_ptr<Hnu::Middleware::Publisher<TaskPlanner::GlobalPathPlan>> publisher;
